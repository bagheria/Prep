...
def markup_record(context, record_text, record_nr, modifiers, targets):
    """ Takes current Patient record, applies context algorithm,
    and appends result to output_dict
    """
    
    # Put in new section in context object:
    # With label of record number
    context.setParent(label="Document")
    context.insertSection(str(record_nr), setToParent = False)    
    print(context.getCurrentparent())
    print(context.getCurrentSectionNumber())
    ...
    context.setParent(label=str(record_nr))
